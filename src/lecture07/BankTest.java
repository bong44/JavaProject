...
public class BankTest {
	public static void main(String[] args) {
		...
		System.out.println(customer3.getFirstName() + "의 SavingsAccount 잔고: " + customer3.getAccounts(0) + "원");
		System.out.println(customer3.getFirstName() + "의 CheckingAccount 잔고: " + customer3.getAccounts(1) + "원");
		System.out.println();
		System.out.println("새로운 withdraw 메소드 실험(1,400원 인출)");
		customer3.getAccounts(1).withdraw(1400);
		System.out.println(customer3.getFirstName() + "의 SavingsAccount 잔고: " + customer3.getAccounts(0) + "원");
		System.out.println(customer3.getFirstName() + "의 CheckingAccount 잔고: " + customer3.getAccounts(1) + "원");
		...
		for (Customer c : bank.getCustomers()) {
			if (c != null) {
				for (int i = 0; i < c.getNumberOfAccounts(); i++) {
					if (c.getAccounts(i) instanceof SavingsAccount) {
						((SavingsAccount) c.getAccounts(i)).updateBalance(10);
					}
				}
			}
		}
	...
	private static void printCustomers(Customer[] customers) {
		for (Customer c : customers) {
			if (c != null) {
				...
				for (int i = 0; i < c.getNumberOfAccounts(); i++) {
					System.out.println("계좌 종류: " + c.getAccounts(i).getClass().getSimpleName() + ", 잔고: " + c.getAccounts(i) + "원");
				}
				System.out.println();
			}
		}
